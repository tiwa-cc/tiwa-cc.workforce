...
- 画面追加とビルド速度のバランスが良い
- Vitest との親和性が高い
- Vite 8 系と Node 24 系で開発環境を揃えやすい
...
### 7. l10n は外部ライブラリ未導入の最小構成で開始

理由:

- まずは文言の直書きを避ける構造を先に作りたい
- `ja` / `en` の切替だけなら Context ベースで十分に小さく始められる
- 将来的に `react-i18next` などへ移行する場合も、文言参照の呼び口を揃えておくと移行しやすい
- 文言ファイルは `shared/i18n/ja` と `shared/i18n/en` に分け、最低限の実務性を確保する

### 8. a11y は lint と状態通知から最小導入する

理由:

- ポートフォリオ段階でも未配慮に見えない最低限の対策が必要
- まずは静的検査と保存中状態の通知を揃える方が、未定義のバリデーションより先に進めやすい
- `aria-invalid` などの入力制約依存の属性は、バリデーション仕様確定後に追加する方が整合しやすい
...
- l10n を将来的に専用ライブラリへ移行するか
- 入力バリデーション仕様と `aria-invalid` の導入基準
